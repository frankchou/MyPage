# Implementation Spec Template (English)
**Source:** Prisma landing page prompt
**Purpose:** Reference template for writing engineer-ready implementation specs
**Key insight:** This style of prompt works because every design decision is already made — the engineer only translates spec to code, zero interpretation needed.
...
Every element specifies the **mechanism**, not the feeling:
- Animation: exact easing `[0.16, 1, 0.3, 1]`, exact delay `0.5s`, exact offset `y: 20`
- Typography: exact vw values, exact tracking, exact line-height
- Color: exact hex codes, exact opacity values
- Layout: exact column counts, exact responsive breakpoints
- Assets: direct URLs — no interpretation needed

The engineer only translates spec → code. Zero design decisions required.
